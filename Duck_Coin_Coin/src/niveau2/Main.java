...
public class Main
{
	...
	static public void main(String[] args)
	{
		...
		System.out.println("Hello. This is the DuckCoinCoin Blockchain Generator Mk2, now with real transactions !");
		affiche_menu();
		while (quitter == false) // boucle principale
		{
			...
			case 4:
				...
				if(!blockchain.isEmpty())
				{
					...
					niveau2.BCJsonUtils.BCJsonWriter(blockchain, "./" + tempo); // s'occupe de traduire en fichier JSON
				} else
					System.out.println("\n\n!!! La blockchain n'existe pas !!! \n!!! Il faut la générer d'abord !!!\n\n");
				affiche_menu();
				break;
			case 5:
				...
				if(!blockchain.isEmpty())
				{
					...
					if(tempo.equals("yes") | tempo.equals("Yes"))
					{
						...
						blockchain = niveau2.BCJsonUtils.BCJsonReader("./" + tempo); // importe la blockchain depuis le fichier JSON
						if(blockchain != null)
						{
							blockchain.afficherBlockchain();
						} else
				...
				blockchain = niveau2.BCJsonUtils.BCJsonReader("./" + tempo);
				if(blockchain != null)
					blockchain.afficherBlockchain();
				else
